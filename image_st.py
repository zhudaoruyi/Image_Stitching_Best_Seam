...
def cylindrical_projection(img, f, vertical=True):
    ...
    w = float(cols / 2.)  # 图像宽度的一半
    h = float(rows / 2.)  # 图像高度的一半
    ...
    for y in range(rows):
        for x in range(cols):
            if vertical:
                ...
                # 水平柱面投影 ==
                point_y = f * (y - h) / math.sqrt(math.pow(y - h, 2) + math.pow(f, 2)) + h
                point_x = f * (x - w) / math.sqrt(math.pow(y - h, 2) + math.pow(f, 2)) + w
            blank[int(point_y), int(point_x), :] = img[y, x, :]
    return blank
...
Usage example:
    ...
    out = cv2.warpPerspective(im1, M, (im2.shape[1], im2.shape[0]), dst=im2.copy(), borderMode=cv2.BORDER_TRANSPARENT)
...
if __name__ == '__main__':
    ...
    if not flag:
        im1 = cv2.imread("data/glacier4/a03.JPG", 0)
        im2 = cv2.imread("data/glacier4/a04.JPG", 0)
        # # show_image("img1", im1)
        # (pts1, pts2) = feature_matching(im1, im2)
        #
        # plt.figure(figsize=(20, 20))
        # plt.subplot(121)
        # plt.imshow(im1)
        # plt.scatter(np.array(pts1)[:, 0], np.array(pts1)[:, 1], 0.5, c='r', marker='x')
        # plt.subplot(122)
        # plt.imshow(im2)
        # plt.scatter(np.array(pts2)[:, 0], np.array(pts2)[:, 1], 0.5, c='r', marker='x')
        # plt.savefig("feature_matching_rgb.jpg", bbox_inches='tight')
        # plt.show()
        # h, w = im1.shape
        # f = 700
        # K = np.array([[f, 0, w/2],
        #               [0, f, h/2],
        #               [0, 0, 1]])
        # imcyl = cylindricalWarpImage(im1, K, savefig=True)
        # plt.show()

        # waved_img = cylindrical_projection(im1, 700, vertical=True)
        # show_image('a11.png', waved_img)
        pad = int(max(im1.shape[0], im1.shape[1], im2.shape[0], im2.shape[1]) * 0.5)

        im2 = cv2.copyMakeBorder(im2, pad, pad, pad, pad, cv2.BORDER_CONSTANT)
        (M, pts1, pts2, mask) = getTransform(im1, im2, method='homography')
        show_image("im2", im2)
        # for example: transform im1 to im2's plane
        # first, make some room around im2
        # then transform im1 with the 3x3 transformation matrix
        # win = max(im1.shape[0], im1.shape[1], im2.shape[0], im2.shape[1]) * 2
        # out = cv2.warpPerspective(im1, M, (win, win))
        # out = cv2.warpPerspective(im1, M, (im2.shape[1], im2.shape[0]))
        out = cv2.warpPerspective(im1, M, (im2.shape[1], im2.shape[0]), dst=im2.copy(),
    ...
    if flag:
        # im1 = cv2.imread("data/glacier4/a03.JPG", 0)
        im1 = cv2.imread("/home/pzw/hdd/dataset/stitch_data/21-43/IMG_180502_044507_0121_RGB.JPG", 0)
        # im2 = cv2.imread("data/glacier4/a04.JPG", 0)
        im2 = cv2.imread("/home/pzw/hdd/dataset/stitch_data/21-43/IMG_180502_044508_0122_RGB.JPG", 0)
        im1 = cv2.resize(im1, (int(im1.shape[1]*0.25), int(im1.shape[0]*0.25)))
        im2 = cv2.resize(im2, (int(im2.shape[1]*0.25), int(im2.shape[0]*0.25)))
        pad = int(max(im1.shape[0], im1.shape[1], im2.shape[0], im2.shape[1]) * 0.5)

        im2 = cv2.copyMakeBorder(im2, pad, pad, pad, pad, cv2.BORDER_CONSTANT)
        (M, pts1, pts2, mask) = getTransform(im1, im2, method='affine')
        show_image("im2", im2)
        out = cv2.warpPerspective(im1, M, (im2.shape[1], im2.shape[0]), dst=im2.copy(),
                                  borderMode=cv2.BORDER_TRANSPARENT)
        show_image("out", out)
